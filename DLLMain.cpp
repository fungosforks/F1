...
DWORD WINAPI dwMainThread ( LPVOID lpArguments )
{
	if ( gInts.Client == NULL ) //Prevent repeat callings.
	{
		...
		gInts.Client = ( IBaseClientDLL* )ClientFactory ( CLIENT_DLL_INTERFACE_VERSION, NULL );
		XASSERT ( gInts.Client );
		gInts.EntList = ( IClientEntityList* )ClientFactory ( VCLIENTENTITYLIST_INTERFACE_VERSION, NULL );
		EngineFactory = ( CreateInterfaceFn )GetProcAddress ( gBaseAPI.GetModuleHandleSafe ( "engine.dll" ), "CreateInterface" );
		gInts.Engine = ( IVEngineClient* )EngineFactory ( VENGINE_CLIENT_INTERFACE_VERSION_13, NULL );
		VGUIFactory = ( CreateInterfaceFn )GetProcAddress ( gBaseAPI.GetModuleHandleSafe ( "vguimatsurface.dll" ), "CreateInterface" );
		gInts.Surface = ( ISurface* )VGUIFactory ( VGUI_SURFACE_INTERFACE_VERSION, NULL );
