...
{
	const VMatrix& worldToScreen = gInts.Engine->WorldToScreenMatrix(); //Grab the world to screen matrix from CEngineClient::WorldToScreenMatrix
...
void CDrawManager::DrawPlayerBox(CBaseEntity *pEnt, DWORD dwColor)
{
	...
	if (gPlayers[pEnt->index].GetFlags() & FL_DUCKING)
	{
	...
	if (WorldToScreen(nom, bot) && WorldToScreen(mon, top))
	{
		...
		OutlineRect(top.x - w, top.y, w * 2, (bot.y - top.y), dwColor);

	}
}

void CDrawManager::DrawPlayerHealthBox(CBaseEntity *pEnt, DWORD dwColor, int health, int maxHealth) 
{
